...
class AVGRunner:
    ...
    def test(self):
        ...
        """
        for ind in range(2):
            batch = get_test_batch(c.BATCH_SIZE, num_rec_out=self.num_test_rec, index=ind+1)
            self.g_model.test_batch(batch, self.global_step, num_rec_out=self.num_test_rec)
            shutil.move("../Save/Images/Default/Tests/Step_0", "../Save/Images/Default/Tests/Step_"+str(ind))
...
def main():
    ...
    load_path = None
    test_only = False
    num_test_rec = 5  # number of recursive predictions to make on test
    num_steps = 1000001
    try:
