...
def get_full_clips(data_dir, num_clips, num_rec_out=1, index=1):
    """
    ...
    all_dirs = glob(os.path.join(data_dir, '*'));
    #ep_dirs = np.random.choice(glob(os.path.join(data_dir, '*')), num_clips)
    ep_dirs = ['../Data/smiletest/'+str(index)]
    print(index)
    print(ep_dirs)
...
def get_test_batch(test_batch_size, num_rec_out=1, index=1):
    """
    ...
    @return: An array of shape:
             ...
             A batch of frame sequences with values normalized in range [-1, 1].
    """
    return get_full_clips(c.TEST_DIR, test_batch_size, num_rec_out=num_rec_out, index=index)
